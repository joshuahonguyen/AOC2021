...
fn main() {
    ...
    for i in init {
        println!("{}", format!("{}{}", i, "="));
        total += add_nodes(i, node_points.clone(), stop.clone(), Vec::new(), "".to_string(), 0);
    }
    println!("{}", total);
}

fn add_nodes(i: String, nodes: Vec<String>, stop: Vec<String>, mut avoid: Vec<String>, mut first_avoid: String, mut total: i32) -> i32 {
    if i == i.to_lowercase() {
        avoid.push(i.clone());
    }
    ...
    for n in &nodes {
        ...
        if n.contains(&i) && !avoid.iter().any(|v| v == &point) {
            total += add_nodes(point, nodes.clone(), stop.clone(), avoid.clone(), first_avoid.clone(), 0);
        }
    }
    return total;
}
